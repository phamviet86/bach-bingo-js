...
/**
 * Xử lý một cặp key-value thành mệnh đề SQL và thêm giá trị vào queryValues
 * @param {string} key - Key của tham số (có thể có hậu tố)
 * @param {any} value - Giá trị của tham số
 * @param {string[]} targetArray - Mảng chứa các mệnh đề SQL
 * @param {any[]} queryValues - Mảng chứa các giá trị tham số
 * @param {string[]} ignoredSearchColumns - Các cột bỏ qua trong mệnh đề WHERE
 * @param {Object} paramIndexRef - Tham chiếu đến chỉ số tham số hiện tại {value: number}
 */
function handleClause(
  ...
  if (key.endsWith("_like")) {
    targetArray.push(
      `unaccent(${baseKey}) ILIKE unaccent($${paramIndexRef.value})`
    );
    queryValues.push(`%${value}%`);
    paramIndexRef.value++;
  } else if (key.endsWith("_nlike")) {
    targetArray.push(
      `unaccent(${baseKey}) NOT ILIKE unaccent($${paramIndexRef.value})`
    );
    queryValues.push(`%${value}%`);
    paramIndexRef.value++;
  } else if (key.endsWith("_e")) {
    targetArray.push(`${baseKey} = $${paramIndexRef.value}`);
    queryValues.push(value);
    paramIndexRef.value++;
    ...
    const placeholders = arr.map(() => `$${paramIndexRef.value++}`);
...
export function parseSearchParams(searchParams, ignoredSearchColumns = []) {
  ...
  let limit = "";
  const queryValues = [];
  const paramIndexRef = { value: ignoredSearchColumns.length + 1 };
